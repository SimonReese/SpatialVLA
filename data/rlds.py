    ...
    def restructure(traj):
        ...
        if state_obs_keys:
            new_obs["proprio"] = tf.concat(
                [
                    (tf.zeros((traj_len, 1), dtype=tf.float32) if key is None else tf.cast(old_obs[key], tf.float32))  # padding
    ...
    *,
    ...
    dataset = dataset.traj_map(
        partial(
            ...
            backward_windows_size=backward_windows_size,
...
def dataset_statistics(
    dataset_kwargs_list: List[Dict], train: bool, sample_weights: Optional[List[float]] = None, balance_weights: bool = False, shuffle_seed = None, **args
):
    ...
    return dataset_len, all_dataset_statistics, sample_weights
...
def build_interleaved_dataset(
    ...
    **args,
    ...
    # Default to uniform sampling (if `sample_weights` is not specified)
    if weights is None:
        weights = [1.0] * len(dataset_kwargs_list)

    if len(weights) != len(dataset_kwargs_list):
        raise ValueError(f"sample_weights must be None or have length {len(dataset_kwargs_list)}.")
    ...
    threads_per_dataset = allocate_threads(traj_transform_threads, weights)
    reads_per_dataset = allocate_threads(traj_read_threads, weights)
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        print(f"make_dataset_from_rlds: threads {threads}, dataset_kwargs {dataset_kwargs}")
    ...
    dataset: dl.DLataset = dl.DLataset.sample_from_datasets(datasets, weights, seed=shuffle_seed)
    ...
    # NOTE: [Contract] When training VLA Policies, we let the Collator handle Batching!
